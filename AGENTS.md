...
**agentcookie.toml emit runs unconditionally for every eligible CLI.** When `.printing-press.json` declares one or more `auth_env_vars`, the sweep emits `library/<cat>/<api>/agentcookie.toml` declaring the CLI as an explicit-manifest adopter at v2 of the agentcookie secrets-bus spec. Cookie-only CLIs (no env-var auth surface) are skipped — agentcookie's cookie adapters handle their auth. Existing `agentcookie.toml` files whose first 1 KiB carries `# agentcookie-manual-override` are preserved untouched so hand-edited manifests survive sweeps. The render is inlined in `tools/sweep-canonical/agentcookie.go` (the sweep tool runs in GOPATH mode, so it doesn't import `github.com/mvanhorn/agentcookie/pkg/agentcookieadoption` directly); the inline format mirrors the helper library's canonical output byte-for-byte so generator-side fresh prints and sweep-side retrofits stay consistent. When `cli-printing-press`'s template change shifts manifest shape, the inline render here must change in lockstep — same pointer-rot rule as the SKILL.md/README.md retrofit dimension.
